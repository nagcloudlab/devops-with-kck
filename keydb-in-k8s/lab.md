...
kubectl config use-context kind-k8s-cluster1
...
### create k8s cluster
```bash
kind create cluster --image kindest/node:v1.30.0 --config kind-config.yaml --name k8s-cluster1
kind create cluster --image kindest/node:v1.30.0 --config kind-config.yaml --name k8s-cluster2
kind get clusters
kubectl get nodes
```
